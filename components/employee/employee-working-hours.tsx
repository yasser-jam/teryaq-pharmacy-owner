...
interface EmplyoeeWorkingHoursProps {
  employee: Employee;
  onClose?: () => void;
}
...
export default function EmployeeWorkingHours({
  ...
  const removeWorkingHoursRequest = (index: number) => {
    setWorkingHoursRequests(workingHoursRequests.filter((_, i) => i !== index));
  };
  ...
  const updateDaysOfWeek = (requestIndex: number, day: DayOfWeek) => {
    ...
    setWorkingHoursRequests((prev) =>
      prev.map((req, idx) =>
        idx === requestIndex
          ? {
              ...req,
              daysOfWeek: isDaySelected
                ? req.daysOfWeek.filter((d) => d !== day)
                : [...req.daysOfWeek, day],
            }
          : req
      )
  ...
  const addShift = (requestIndex: number) => {
    ...
    setWorkingHoursRequests(updatedRequests);
  ...
  const handleSubmit = () => {
    ...
    if (hasEmptyDays) {
      setValidationErrors([
        ...validationErrors,
        { requestIndex: -1, shiftIndex: -1, message: "Please select at least one day for each schedule type" }
      ]);
      return;
    }
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {workingHoursRequests.length > 1 && (
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            removeWorkingHoursRequest(requestIndex)
                          }
                        ...
                        {DAYS_OF_WEEK.map((day) => {
                          ...
                          return (
                            <Badge
                              ...
                              onClick={() =>
                                !isDisabled &&
                                updateDaysOfWeek(requestIndex, day)
                              }
                            >
                              {DAY_LABELS[day]}
                            </Badge>
